...
nt = 3*length(TI);     % number of frames. For each TI: tag, control, and velocity-mimicking
...
% shift data by a sample to correct for adc-gradient shift (by eyeballing rotation)
d = d(2:ndat,:,:,:,:);
kx = readout.kx(1:ndat-1,:);
ky = -readout.ky(1:ndat-1,:);  % rotations are negated, need to look into this TODO
